...
- [ ] 1. Add GraveyardSettings to AppState
  - [ ] 1.1 Create GraveyardSettings struct in src/app.rs
  ...
  - [ ] 1.2 Create LatencyConfig struct in src/app.rs
...
- [ ] 3. Update status bar with toggle indicators
  - [ ] 3.1 Add toggle status display in render_status_bar()
    ...
    - _Requirements: 5.6_
  - [ ] 3.2 Add keyboard hints for new toggles
    - Add "A:Anim" hint
    - Add "H:Theme" hint
    - Add "t:Labels" hint
    - _Requirements: 5.6_

- [ ] 4. Checkpoint - Verify toggle foundation
  - Ensure all tests pass, ask the user if questions arise.

## Phase 2: Latency Rings and Position Calculation

- [ ] 5. Implement latency bucket classification
  - [ ] 5.1 Create classify_latency() function in src/ui.rs
    - Input: latency_ms (Option<u64>), config (LatencyConfig)
    - Return: LatencyBucket enum
    - Handle None case → LatencyBucket::Unknown
    - _Requirements: 1.2, 1.3, 1.4, 1.5_
  - [ ] 5.2 Add latency data to endpoint aggregation
    - Extend EndpointNode struct with `latency_bucket: LatencyBucket`
    - Calculate bucket during endpoint collection
    - _Requirements: 1.1_

- [ ] 6. Draw latency rings on canvas
  - [ ] 6.1 Create draw_latency_rings() function in src/ui.rs
  ...
  - [ ] 6.2 Conditionally render rings
    - Only draw rings if at least one endpoint has latency data
    - Skip rings if all endpoints are LatencyBucket::Unknown
    - _Requirements: 1.5_

- [ ] 7. Update endpoint position calculation
  - [ ] 7.1 Modify calculate_endpoint_position() in src/ui.rs
    ...
    - _Requirements: 1.2, 1.3, 1.4_
  - [ ] 7.2 Implement fallback for unknown latency
    - Use middle ring (radius 25) as default
    - Maintain existing radial layout behavior
    - _Requirements: 1.5_

- [ ] 8. Checkpoint - Verify latency rings
  - Ensure all tests pass, ask the user if questions arise.

## Phase 3: Edge Particle Animation

- [ ] 9. Implement particle position calculation
  - [ ] 9.1 Create particle_position() function in src/ui.rs
