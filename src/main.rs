...
fn ui(f: &mut Frame) {
    ...
    render_network_map(f, body_chunks[0]);
    ...
    render_soul_inspector(f, right_chunks[0]);
    render_grimoire(f, right_chunks[1]);
...
fn render_network_map(f: &mut Frame, area: Rect) {
    let map_content = vec![
        Line::from(""),
        Line::from(vec![
            Span::raw("          "),
...
fn render_soul_inspector(f: &mut Frame, area: Rect) {
...
fn render_grimoire(f: &mut Frame, area: Rect) {
    let log_items = vec![
